...
async function runTest() {
  ...
  const { data: accounts, error: accError } = await supabase.from("trading_accounts").select("*").limit(1);
  if (accError) {
  ...
  const { data: checkins, error: checkinsError } = await supabase.from("daily_checkins").select("*").limit(1);
  if (checkinsError) {
